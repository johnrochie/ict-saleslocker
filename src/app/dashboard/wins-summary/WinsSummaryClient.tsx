...
// Internal/test accounts that shouldn't count toward real win figures.
...
export default function WinsSummaryClient({ all, year }: { all: Opportunity[]; year: number }) {
  ...
  // Won deals whose created_date falls inside the selected range.
  const won = useMemo(() => {
  ...
  const metrics = useMemo(() => ({
    wonCount: won.length,
    wonRev:   won.reduce((s, o) => s + o.revenue_total, 0),
    avgDeal:  won.length ? won.reduce((s, o) => s + o.revenue_total, 0) / won.length : 0,
  }), [won])
  ...
  const stageRows = useMemo(() => {
    ...
    won.forEach(o => {
      ...
      if (!map.has(key)) map.set(key, { count: 0, rev: 0 })
  ...
  const catRows = useMemo(() => {
    const map = new Map<string, { count: number; rev: number }>()
    const total = won.reduce((s, o) => s + o.revenue_total, 0)
    won.forEach(o => {
  ...
  return (
    <div style={{ fontFamily: "'Arial', sans-serif", fontSize: 13, color: '#1e293b', background: '#f1f5f9', minHeight: '100vh', margin: -24 }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#1A3A5C 0%,#142d47 100%)', color: 'white', padding: '12px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 17, fontWeight: 800, letterSpacing: -.3 }}>
            ICT Services &nbsp;—&nbsp; <span style={{ color: '#93c5fd' }}>Sales Wins Summary</span>
          </h1>
          <p style={{ margin: '2px 0 0', fontSize: 11, color: 'rgba(255,255,255,.45)' }}>
            {metrics.wonCount} won {periodLabel}
          </p>
        </div>
      ...
      <div style={{ background: '#1e3a5f', color: 'white', padding: '7px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: .5, textTransform: 'uppercase' }}>Won Deals — By Stage &amp; Category</span>
      ...
      <div className="ps-wrap" style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 28px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
          ...
          <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            ...
            <table className="ps-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
              <tbody>
                {[
                  { label: 'Total won opportunities', val: metrics.wonCount.toLocaleString('en-IE'), bold: false },
                  { label: 'Total won value',          val: euros(metrics.wonRev),                     bold: true, color: '#16a34a' },
                  { label: 'Avg won deal size',        val: euros(metrics.avgDeal),                     bold: false },
                ].map(row => (
                  <tr key={row.label}>
            ...
            <table className="ps-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                ...
                <tr style={{ background: '#1e3a5f' }}>
                  ...
                  <td className="r" style={{ fontWeight: 800, color: 'white' }}>{metrics.wonCount}</td>
                  <td className="r" style={{ fontWeight: 800, color: 'white' }}>{euros(metrics.wonRev)}</td>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            ...
            <table className="ps-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {stageRows.map(row => (
                  <tr key={row.name}>
                    <td style={{ color: '#1e293b' }}>{row.name}</td>
                    <td className="r" style={{ color: '#64748b' }}>{row.count}</td>
                    <td className="r" style={{ fontWeight: 700 }}>{euros(row.rev)}</td>
                  </tr>
                ))}
          ...
          {/* CHART */}
          <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            <div style={{ background: '#1e3a5f', color: 'white', padding: '8px 14px', fontSize: 11, fontWeight: 700, letterSpacing: .5, textTransform: 'uppercase' }}>Won Value by Stage</div>
            <div style={{ padding: '16px 20px' }}>
              {chartRows.length === 0
                ? <p style={{ textAlign: 'center', color: '#94a3b8', fontSize: 12, padding: '20px 0' }}>No won deals in this period</p>
