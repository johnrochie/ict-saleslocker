...
interface CompanySplit {
  id: string; company: string; rep1_name: string; rep1_pct: number
  rep2_name: string | null; rep2_pct: number | null; notes: string | null
}
...
function RateInput({ value, onSave }: { value: number; onSave: (v: number) => void }) {
  ...
  if (!editing) {
    return (
      <button onClick={() => setEditing(true)} className="text-sm font-semibold text-brand-600 hover:underline tabular-nums">
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      <input type="number" value={draft} step="0.01" min="0" max="100" onChange={e => setDraft(e.target.value)}
        className="w-20 rounded border border-brand-300 px-2 py-0.5 text-sm text-right focus:outline-none focus:ring-1 focus:ring-brand-500" autoFocus />
        ...
        <div className="border-t border-gray-100 px-5 py-4 space-y-4">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="flex items-end gap-3">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={draft.is_active} onChange={e => update('is_active', e.target.checked)} className="rounded" />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={draft.quarterly_threshold ?? ''}
                    ...
                    className="w-full rounded-lg border border-gray-200 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" placeholder="18000" />
                </div>
                <div>
          ...
          {draft.commission_type === 'type3' && (
            <div className="bg-green-50 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={draft.annual_margin_target ?? ''}
                    ...
                    className="w-full rounded-lg border border-gray-200 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" placeholder="450000" />
                </div>
                <div>
                  ...
                  <input type="number" value={draft.annual_bonus ?? ''}
                    ...
                    className="w-full rounded-lg border border-gray-200 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" placeholder="56000" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Stage 1 Start</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Stage 2 Start</label>
                  <input type="number" value={draft.stage2_threshold ?? ''}
                    onChange={e => update('stage2_threshold', parseFloat(e.target.value))}
                    className="w-full rounded-lg border border-gray-200 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" placeholder="950000" />
                </div>
                <div>
              ...
              <div className="mt-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={draft.rollover_enabled} onChange={e => update('rollover_enabled', e.target.checked)} className="rounded" />
                  <span className="text-sm text-gray-600">Enable quarterly rollover</span>
                </label>
              </div>
            </div>
          )}
...
export default function CommissionSettingsClient({
  categoryMappings, type1Rates, repConfigs, companySplits, currentUserEmail,
}: {
  categoryMappings: CategoryMapping[]
  type1Rates: Type1Rate[]
  repConfigs: RepConfig[]
  companySplits: CompanySplit[]
  currentUserEmail: string
}) {
  const [tab, setTab] = useState<'categories' | 'rates' | 'reps' | 'splits'>('categories')
  ...
  const [splits, setSplits] = useState(companySplits)
  const [saving, setSaving] = useState(false)
  const [msg, setMsg]       = useState<{ type: 'ok' | 'err'; text: string } | null>(null)
  const [showAddRep, setShowAddRep] = useState(false)
  const [newRep, setNewRep] = useState({ autotask_name: '', display_name: '', commission_type: 'type1' })
  const [showAddSplit, setShowAddSplit] = useState(false)
  const [newSplit, setNewSplit] = useState({ company: '', rep1_name: '', rep1_pct: 50, rep2_name: '', rep2_pct: 50 })

  const repNames = reps.filter(r => r.is_active).map(r => r.autotask_name)
  ...
  async function saveSplit() {
    if (!newSplit.company.trim() || !newSplit.rep1_name.trim()) return
    setSaving(true)
    const res = await fetch('/api/commission/splits', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        company: newSplit.company.trim(),
        rep1_name: newSplit.rep1_name.trim(),
        rep1_pct: newSplit.rep1_pct,
        rep2_name: newSplit.rep2_name.trim() || null,
        rep2_pct: newSplit.rep2_name ? newSplit.rep2_pct : null,
    ...
    if (res.ok) {
      setSplits(prev => {
        const idx = prev.findIndex(s => s.company === newSplit.company)
        return idx >= 0 ? prev.map((s, i) => i === idx ? data.split : s) : [...prev, data.split]
      })
      setNewSplit({ company: '', rep1_name: '', rep1_pct: 50, rep2_name: '', rep2_pct: 50 })
      setShowAddSplit(false)
      flash('ok', 'Split saved')
    } else {
  ...
  async function deleteSplit(id: string) {
    ...
    if (res.ok) { setSplits(prev => prev.filter(s => s.id !== id)); flash('ok', 'Split removed') }
  ...
  const tabs = [
    ...
    { id: 'splits',     label: 'Account Splits' },
  ] as const
    ...
    <div className="space-y-6">
      ...
      {tab === 'categories' && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            ...
            <p className="text-xs text-gray-400 mt-0.5">Control how each Autotask category is treated. Set to Exclude to skip categories that should not earn commission.</p>
          </div>
              ...
              {cats.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3">
                    <select value={c.commission_category}
                      onChange={(e) => {
                      ...
                      {COMM_CATEGORIES.map(cat => <option key={cat} value={cat}>{commCatLabel(cat)}</option>)}
                    </select>
                  </td>
      ...
      {tab === 'rates' && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            ...
            <p className="text-xs text-gray-400 mt-0.5">Rates applied to burdened margin. Click a rate to edit inline.</p>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-xs font-semibold text-gray-400 uppercase tracking-wide">
              ...
              {rates.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  <td className="px-5 py-3 font-medium text-gray-800">
                    {{ new_client: 'New Client', existing_client: 'Existing Client', renewal: 'Renewal' }[r.business_type] ?? r.business_type}
                  </td>
                  <td className="px-5 py-3 text-gray-600">{commCatLabel(r.commission_category)}</td>
                  <td className="px-5 py-3 text-right">
                    <RateInput value={r.rate_pct} onSave={(v) => {
      ...
      {tab === 'reps' && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={() => setShowAddRep(true)}
              ...
              <div className="grid grid-cols-3 gap-3 mb-3">
                ...
                <div>
                  ...
                  <select value={newRep.commission_type} onChange={e => setNewRep(p => ({ ...p, commission_type: e.target.value }))}
                    ...
                    {['type1','type2','type3'].map(t => <option key={t} value={t}>{commTypeLabel(t)}</option>)}
                  </select>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={addRep} disabled={saving}
                  className="rounded-lg bg-brand-500 text-white px-4 py-1.5 text-sm font-medium hover:bg-brand-600 disabled:opacity-60">Save</button>
                <button onClick={() => setShowAddRep(false)}
                  className="rounded-lg border border-gray-200 px-4 py-1.5 text-sm text-gray-600 hover:bg-gray-50">Cancel</button>
              </div>
            </div>
          )}
          {reps.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-8 text-center text-gray-400 text-sm">No reps configured. Add a rep above.</div>
          ) : (
            reps.map((rep) => (
              <RepConfigCard key={rep.id} rep={rep}
                onSave={(updated) => { setReps(prev => prev.map(r => r.id === updated.id ? updated : r)); saveRepConfig(updated) }} />
            ))
          )}
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-start justify-between mb-2">
              <div>
                <h2 className="text-sm font-semibold text-navy-700">Account Splits</h2>
                <p className="text-xs text-gray-400 mt-0.5">
                  Define default commission splits for shared accounts. These apply automatically to all won deals for that company.
                  Individual deals can still be overridden in the Commission Review.
                </p>
              </div>
              <button onClick={() => setShowAddSplit(true)}
                className="rounded-lg bg-brand-500 text-white px-4 py-2 text-sm font-semibold hover:bg-brand-600 transition-colors shrink-0 ml-4">
                + Add Split
              </button>
            </div>
            ...
            {showAddSplit && (
              <div className="mt-4 p-4 rounded-lg bg-gray-50 border border-gray-200">
                <h3 className="text-sm font-semibold text-navy-700 mb-3">Add company split</h3>
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div className="col-span-2">
                    <label className="block text-xs font-medium text-gray-500 mb-1">Company Name (exact, as it appears in Autotask)</label>
                    <input value={newSplit.company} onChange={e => setNewSplit(p => ({ ...p, company: e.target.value }))}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
                      placeholder="RenaissanceRe" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">Rep 1</label>
                    <select value={newSplit.rep1_name} onChange={e => setNewSplit(p => ({ ...p, rep1_name: e.target.value }))}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">Rep 1 % share</label>
                    <input type="number" min="1" max="100" value={newSplit.rep1_pct}
                      onChange={e => { const v = parseInt(e.target.value); setNewSplit(p => ({ ...p, rep1_pct: v, rep2_pct: 100 - v })) }}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">Rep 2 (optional)</label>
                    <select value={newSplit.rep2_name} onChange={e => setNewSplit(p => ({ ...p, rep2_name: e.target.value }))}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-500">
                      <option value="">None (100% to Rep 1)</option>
                      {repNames.map(n => <option key={n} value={n}>{n}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">Rep 2 % share</label>
                    <input type="number" min="0" max="99" value={newSplit.rep2_pct} disabled={!newSplit.rep2_name}
                      onChange={e => { const v = parseInt(e.target.value); setNewSplit(p => ({ ...p, rep2_pct: v, rep1_pct: 100 - v })) }}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 disabled:opacity-40" />
                  </div>
                </div>
                {newSplit.rep1_name && newSplit.rep2_name && (
                  <p className="text-xs text-gray-500 mb-3">
                    Split: {newSplit.rep1_pct}% to {newSplit.rep1_name} / {newSplit.rep2_pct}% to {newSplit.rep2_name}
                  </p>
                )}
                <div className="flex gap-2">
                  <button onClick={saveSplit} disabled={saving || !newSplit.company || !newSplit.rep1_name}
                    className="rounded-lg bg-brand-500 text-white px-4 py-1.5 text-sm font-medium hover:bg-brand-600 disabled:opacity-60">Save</button>
          ...
          {splits.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-8 text-center text-gray-400 text-sm">
              No account splits configured. Add a split above to set up shared commission accounts.
            </div>
          ) : (
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 text-xs font-semibold text-gray-400 uppercase tracking-wide">
                    ...
                    <th className="text-left px-5 py-3">Rep 1</th>
                    <th className="text-left px-5 py-3">Split</th>
                    <th className="text-left px-5 py-3">Rep 2</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {splits.map((s) => (
                    <tr key={s.id} className="hover:bg-gray-50">
                      <td className="px-5 py-3 font-medium text-gray-800">{s.company}</td>
                      <td className="px-5 py-3 text-gray-700">{s.rep1_name}</td>
                      <td className="px-5 py-3">
                        <span className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 text-xs font-semibold px-2 py-0.5 rounded-full">
                          {s.rep1_pct}% / {s.rep2_pct ?? 0}%
                        </span>
                      </td>
                      <td className="px-5 py-3 text-gray-600">{s.rep2_name || <span className="text-gray-300">—</span>}</td>
                      <td className="px-5 py-3 text-right">
                        <button onClick={() => deleteSplit(s.id)} className="text-xs text-red-500 hover:underline font-medium">Remove</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
